resource "aws_lambda_function" "count_running_executions" {
  function_name = "${format("%.64s", "ecs-inst-rdy_count_running-${var.autoscaling_group_name}")}"
  description   = "ECS instance readiness poller - count-running-executions for ${var.autoscaling_group_name} Auto Scaling Group"
...
resource "aws_iam_role" "count_running_executions" {
  name               = "${format("%.64s", "ecs-inst-rdy-count_running-${var.autoscaling_group_name}")}"
