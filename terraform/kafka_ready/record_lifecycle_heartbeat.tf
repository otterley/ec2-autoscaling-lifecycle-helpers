resource "aws_lambda_function" "record_lifecycle_heartbeat" {
  function_name = "${format("%.64s", "kafka-rdy-lc-htbt-${var.autoscaling_group_name}")}"
  description   = "Kafka readiness poller - record_lifecycle_heartbeat for ${var.autoscaling_group_name} group"
...
resource "aws_iam_role" "record_lifecycle_heartbeat" {
  name               = "${format("%.64s", "kafka-rdy-lc-htbt-${var.autoscaling_group_name}")}"
