resource "aws_sfn_state_machine" "poller" {
  name     = "kafka_ready_poller-${var.autoscaling_group_name}"
  role_arn = "${aws_iam_role.poller.arn}"

  definition = <<EOF
{
    "Comment": "Kafka readiness poller - ${var.autoscaling_group_name}",
    ...
    "States": {
        ...
        "CheckReady": {
            "Type": "Task",
            "Resource": "${aws_lambda_function.check_kafka_ready.arn}",
...
data "aws_iam_policy_document" "poller_policy" {
  statement {
    ...
    resources = [
      ...
      "${aws_lambda_function.check_kafka_ready.arn}",
