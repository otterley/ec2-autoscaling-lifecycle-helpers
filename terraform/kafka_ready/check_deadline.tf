resource "aws_lambda_function" "check_deadline" {
  function_name = "${format("%.64s", "kafka-rdy-chk-dead-${var.autoscaling_group_name}")}"
  description   = "Kafka readiness poller - check_deadline for ${var.autoscaling_group_name} group"
...
resource "aws_iam_role" "check_deadline" {
  name               = "${format("%.64s", "kafka-rdy-chk-dead-${var.autoscaling_group_name}")}"
