resource "aws_lambda_function" "complete_lifecycle_action" {
  function_name = "${format("%.64s", "ecs-inst-drain-lc-act-${var.autoscaling_group_name}")}"
  description   = "ECS instance drainer - complete_lifecycle_action for ${var.autoscaling_group_name} group"
...
resource "aws_iam_role" "complete_lifecycle_action" {
  name               = "${format("%.64s", "ecs-inst-drain-lc-act-${var.autoscaling_group_name}")}"
