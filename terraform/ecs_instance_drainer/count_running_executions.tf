resource "aws_lambda_function" "count_running_executions" {
  function_name = "${format("%.64s", "ecs_drain_count_running-${var.autoscaling_group_name}")}"
  description   = "ECS instance drainer - count-running-executions for ${var.autoscaling_group_name} Auto Scaling Group"
...
resource "aws_iam_role" "count_running_executions" {
  name               = "${format("%.64s", "ecs-inst-drain-count_running-${var.autoscaling_group_name}")}"
